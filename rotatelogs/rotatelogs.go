...
import (
	...
	"github.com/pemako/gopkg/rotatelogs/internal/errors"
...
func (rl *RotateLogs) getWriterNolock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
	...
	if err != nil {
		return nil, errors.Errorf(`failed to create a new file %v:%v`, filename, err)
	}
...
func (rl *RotateLogs) rotateNolock(filename string) error {
	...
	if rl.linkName != "" {
		...
		if strings.Contains(rl.linkName, baseDir) {
			...
			if err != nil {
				return errors.Errorf(`failed to evaluate relative path from %#v to %#v err %s`, baseDir, rl.linkName, err)
			}
		...
		if err != nil { // Assume err != nil means the directory doesn't exist
			if err := os.MkdirAll(linkDir, 0755); err != nil {
				return errors.Errorf(`failed to create directory %s err:%s`, linkDir, err)
			}
		}
