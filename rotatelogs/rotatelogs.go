...
import (
	...
	"github.com/pemako/gopkg/rotatelogs/internal/fileutil"
	"github.com/pemako/gopkg/strftime"
)
...
func New(p string, options ...Option) (*RotateLogs, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(`invalid strftime pattern %s`, err)
	}
...
func (rl *RotateLogs) Write(p []byte) (n int, err error) {
	...
	if err != nil {
		return 0, fmt.Errorf(`failed to acquite target io.Writer %s`, err)
	}
...
func (rl *RotateLogs) getWriterNolock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(`failed to create a new file %v %s`, err, filename)
	}

	if err := rl.rotateNolock(filename); err != nil {
		err = fmt.Errorf("failed to rotate %s", err)
		if bailOnRotateFail {
...
func (rl *RotateLogs) rotateNolock(filename string) error {
	...
	if rl.linkName != "" {
		...
		if strings.Contains(rl.linkName, baseDir) {
			...
			if err != nil {
				return fmt.Errorf(`failed to evaluate relative path from %#v to %#v %s`, baseDir, rl.linkName, err)
			}
		...
		if err != nil { // Assume err != nil means the directory doesn't exist
			if err := os.MkdirAll(linkDir, 0755); err != nil {
				return fmt.Errorf(`failed to create directory %s %s`, linkDir, err)
			}
		}
