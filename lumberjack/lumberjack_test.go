...
func TestDefaultFilename(t *testing.T) {
	currentTime = fakeTime
	dir := os.TempDir()
	filename := filepath.Join(dir, filepath.Base(os.Args[0])+"-lumberjack.lumberjack")
...
func TestMaxBackups(t *testing.T) {
	...
	// Make a directory that exactly matches our lumberjack file filters... it still
	...
	// Create a lumberjack file that is/was being compressed - this should
	// not be counted since both the compressed and the uncompressed
	// lumberjack files still exist.
	...
	// We should have four things in the directory now - the 2 lumberjack files, the
	// not lumberjack file, and the directory
	fileCount(dir, 5, t)
...
func TestCleanupExistingBackups(t *testing.T) {
	...
	// now create a primary lumberjack file with some data
	filename := logFile(dir)
	err = os.WriteFile(filename, data, 0644)
	isNil(err, t)
...
func TestMaxAge(t *testing.T) {
	...
	// We should still have 2 lumberjack files, since the most recent backup was just
	// created.
	fileCount(dir, 2, t)
	...
	// We should have 2 lumberjack files - the main lumberjack file, and the most recent
...
func TestTimeFromName(t *testing.T) {
	l := &Logger{Filename: "/var/lumberjack/myfoo/foo.lumberjack"}
	...
	tests := []struct {
		...
		{"foo-2014-05-04T14-44-33.555.lumberjack", time.Date(2014, 5, 4, 14, 44, 33, 555000000, time.UTC), false},
		{"foo-2014-05-04T14-44-33.555", time.Time{}, true},
		{"2014-05-04T14-44-33.555.lumberjack", time.Time{}, true},
		{"foo.lumberjack", time.Time{}, true},
	}
...
func TestCompressOnRotate(t *testing.T) {
	...
	// a compressed version of the lumberjack file should now exist and the original
...
func TestCompressOnResume(t *testing.T) {
	...
	// the lumberjack file should now exist and the original should have been removed.
	bc := new(bytes.Buffer)
	gz := gzip.NewWriter(bc)
	_, err = gz.Write(b)
	isNil(err, t)
...
// logFile returns the lumberjack file name in the given directory for the current fake
// time.
func logFile(dir string) string {
	return filepath.Join(dir, "foobar.lumberjack")
}

func backupFile(dir string) string {
	return filepath.Join(dir, "foobar-"+fakeTime().UTC().Format(backupTimeFormat)+".lumberjack")
}

func backupFileLocal(dir string) string {
	return filepath.Join(dir, "foobar-"+fakeTime().Format(backupTimeFormat)+".lumberjack")
}

// logFileLocal returns the lumberjack file name in the given directory for the current
